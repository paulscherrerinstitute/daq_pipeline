...
size_t bsread::get_compression_buffer_size(
        ...
        size_t element_size)
{
    switch (compression) {
        ...
        case compression_bslz4:
            return bshuf_compress_lz4_bound(
                    n_elements, element_size, 0) + 12;

        default:
            throw runtime_error("Cannot determine compression buffer size "
                                "for unknown compression type.");
    }
}

size_t bsread::compress_buffer(
        compression_type compression,
        const char* data,
        size_t n_elements,
        size_t element_size,
        char* buffer, size_t buffer_size)
{
    switch (compression) {
        ...
        case compression_lz4: {
            return compress_lz4(
                    data, n_elements, element_size, buffer, buffer_size);
        }
        ...
        default:
            throw runtime_error("Cannot compress with "
                                "unknown compression type.");
    }
}

size_t bsread::decompress_buffer(
        compression_type compression,
        ...
        char* data)
{
    switch (compression) {
        ...
        case compression_bslz4:
            return decompress_bitshuffle(compressed_data, compressed_size,
                                         n_elements, element_size, data);

        default:
            throw runtime_error("Cannot decompress with "
                                "unknown compression type.");
    }
}

size_t bsread::compress_lz4(
        const char* data,
        size_t n_elements,
        size_t element_size,
        char* buffer, size_t buffer_size)
{
    ...
    size_t compressed_size = LZ4_compress_default(
            data, &buffer[4], data_len, buffer_size-4);
...
size_t bsread::compress_bitshuffle(
        const char* data,
        size_t n_elements,
        size_t element_size,
        char* buffer)
{
    ...
    auto compressed_size = bshuf_compress_lz4(
            data, &buffer[12], n_elements, element_size, block_size);
...
size_t bsread::decompress_bitshuffle(
        const char* compressed_data,
        size_t compressed_size,
        size_t n_elements,
        size_t element_size,
        char* data)
{
    ...
    auto n_processed_bytes = bshuf_decompress_lz4(
            &compressed_data[12], data,
            n_elements, element_size, block_size);
